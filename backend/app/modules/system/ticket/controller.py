...
async def ticket_list_controller(
    ...
    result: PageResultSchema[TicketOutSchema] = await TicketService(auth, db).page(
...
async def ticket_detail_controller(
    ...
    result: TicketOutSchema = await TicketService(auth, db).detail(id=id)
    return SuccessResponse(data=result, msg="查询成功")
...
async def ticket_create_controller(
    ...
    result: TicketOutSchema = await TicketService(auth, db).create(data=data)
...
async def ticket_update_controller(
    ...
    result: TicketOutSchema = await TicketService(auth, db).update(id=id, data=data)
...
async def ticket_export_controller(
    ...
    ticket_list: list[TicketOutSchema] = await TicketService(auth, db).get_list(search=search)
    export_result: bytes = await TicketService.export_list(ticket_list=[item.model_dump() for item in ticket_list])
...
async def ticket_comment_list_controller(
    ...
    result: PageResultSchema[TicketCommentOutSchema] = await TicketCommentService(auth, db).page(ticket_id=ticket_id, page_no=page.page_no, page_size=page.page_size)
    return SuccessResponse(data=result, msg="查询成功")
...
async def ticket_comment_create_controller(
    ...
    result: TicketCommentOutSchema = await TicketCommentService(auth, db).create(ticket_id=ticket_id, data=data)
