...
from app.api.v1.module_task.workflow.flow.schema import WorkflowFlowCreateSchema, WorkflowFlowExecuteSchema, WorkflowFlowOutSchema, WorkflowFlowQueryParam, WorkflowFlowUpdateSchema
from app.api.v1.module_task.workflow.flow.service import WorkflowFlowService
...
async def get_flow_page_controller(
    ...
    result = await WorkflowFlowService(auth, db).page(
        search=search,
...
async def get_flow_list_controller(
    ...
    result = await WorkflowFlowService(auth, db).get_list(search=search)
...
async def get_flow_detail_controller(
    ...
    result = await WorkflowFlowService(auth, db).detail(id=id)
...
async def create_flow_controller(
    ...
    result = await WorkflowFlowService(auth, db).create(data=data)
...
async def update_flow_controller(
    ...
    result = await WorkflowFlowService(auth, db).update(id=id, data=data)
...
async def execute_flow_controller(
    ...
    task_ids = await WorkflowFlowService(auth, db).execute(
