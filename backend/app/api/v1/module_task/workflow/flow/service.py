...
from app.api.v1.module_task.workflow.node.model import StorageSourceModel
from app.api.v1.module_task.workflow.node.service import StorageSourceService
from app.api.v1.module_task.workflow.transfer.schema import TransferTargetSchema, TransferTaskCreateSchema
from app.api.v1.module_task.workflow.transfer.service import StorageTransferService
from app.core.base_schema import AuthSchema, PageResultSchema
from app.core.exceptions import CustomException
from app.utils.common_util import search_to_dict

from .crud import WorkflowFlowCRUD
...
class WorkflowFlowService:
    ...
    async def _save_graph(self, flow_id: int, nodes: list[FlowNodeSchema], edges: list[FlowEdgeSchema]) -> None:
        ...
        await self.db.execute(delete(WorkflowFlowNodeModel).where(WorkflowFlowNodeModel.flow_id == flow_id))
        await self.db.execute(delete(WorkflowFlowEdgeModel).where(WorkflowFlowEdgeModel.flow_id == flow_id))
    ...
    async def _build_graph(
        ...
        if ids:
            result = await self.db.execute(
                select(StorageSourceModel).where(
                    StorageSourceModel.id.in_(ids),
                    StorageSourceModel.is_deleted.is_(False),
                )
            )
            src_map = {s.id: s for s in result.scalars().all()}
        ...
        for key, le in layout_edges.items():
            ...
            edge["data"] = FlowGraphEdgeDataSchema(
                ...
                source_label=src.name if src else (src_node.source_id if src_node else None),
                target_label=tgt.name if tgt else (tgt_node.source_id if tgt_node else None),
    ...
    async def update(self, id: int, data: WorkflowFlowUpdateSchema) -> WorkflowFlowOutSchema:
        ...
        data_dict, node_items, edge_items = await self._prepare_and_save(id, data)
        await self._save_graph(id, node_items, edge_items)
    ...
    async def delete(self, ids: list[int]) -> None:
        ...
        await self.db.execute(delete(WorkflowFlowNodeModel).where(WorkflowFlowNodeModel.flow_id.in_(ids)))
        await self.db.execute(delete(WorkflowFlowEdgeModel).where(WorkflowFlowEdgeModel.flow_id.in_(ids)))
